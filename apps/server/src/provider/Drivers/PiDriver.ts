/**
 * PiDriver — driver for the pi (pi-coding-agent) provider.
 *
 * pi is a standalone CLI coding agent. This driver spawns one
 * `pi --mode rpc` process per active thread (see PiAdapter), probes
 * `pi --version` for health state, and offers pi-backed text generation
 * for commit messages, PR content, branch names, and thread titles.
 *
